...
let bot;
let MarkovGeneratorWord;
let corpus;
let randomResponseChance;
let randomResponseCooldown;
let ignoreBots;
let learnFromBots;
let ignoreDirectMessages;
let learnFromDirectMessages;
...
module.exports = {
  ...
  init: (client, corpusFile) => {
    ...
    const lines = fs.readFileSync(corpus).toString().split('\n');
  ...
  learn: (data) => {
    try {
      let guild; let channel; let author; let message; let lines; let
        text;
      guild = data[0];
      channel = data[1];
      author = data[2];
  ...
  storeGif: (url) => {
    try {
      fs.appendFile(corpus.replace('.log', '.gifs'), `${url}\n`, (err) => {
        if (err) console.log(err);
      });
    } catch (error) {
      console.log(error);
